...
Date: 22/09/2017
Description: Using XE to monitor deadlock 
...
IF EXISTS ( SELECT  1
            ...
            WHERE   name = 'Monitoring_Deadlocks' )
    DROP EVENT SESSION [Monitoring_Deadlocks] ON SERVER;
GO

CREATE EVENT SESSION [Monitoring_Deadlocks] ON SERVER
ADD EVENT sqlserver.lock_deadlock (  SET collect_database_name = ( 1 ) ,
...
ADD EVENT sqlserver.lock_deadlock_chain (  SET collect_database_name = ( 1 ) ,
                                           collect_resource_description = ( 1 )
...
ADD TARGET package0.event_file (  SET filename = 'Monitoring_Deadlocks.xel' ,
                                  max_file_size = 250 ,
...
ALTER EVENT SESSION [Monitoring_Deadlocks] ON SERVER STATE = START;

WITH    cte
          AS ( SELECT   t2.event_data.value('(event/@name)[1]', 'varchar(50)') AS event_name ,
