...
Date: 22/09/2017
Description: Max Server Memory Calculator
Original link: https://bornsql.ca/memory/
...
SELECT  @cpuArchitecture = CASE
                               WHEN @@VERSION LIKE '%<X64>%' THEN 2
                               WHEN @@VERSION LIKE '%<IA64>%' THEN 4
                               ELSE 0.5
                           END
...
SELECT  @@VERSION AS [Version]
      , CASE
            WHEN ( @enterprise = 1 ) THEN 'Enterprise Edition'
            WHEN ( @developer = 1 ) THEN 'Developer Edition'
      ...
      , N'EXEC sp_configure ''show advanced options'', 1; RECONFIGURE WITH OVERRIDE; EXEC sp_configure ''max server memory (MB)'', '
        + CAST(CAST(@recommendedMemory * 1024 AS INT) AS NVARCHAR (20)) + '; EXEC sp_configure ''show advanced options'', 0; RECONFIGURE WITH OVERRIDE;' AS [Script]
