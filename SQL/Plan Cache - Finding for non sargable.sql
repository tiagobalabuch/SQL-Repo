...
Author: Bert Wagner
Date : 22/09/2017
Description: Search for non-sargable queries
...
SELECT  stmt.value('(@StatementText)[1]' , 'varchar(max)') AS [Query]
      ...
      , CASE WHEN s.exist('.//TableScan') = 1 THEN 'TableScan'
             ELSE 'IndexScan'
        END AS [ScanType]
...
WHERE   s.exist('.//ScalarOperator[@ScalarString]!=""') = 1 AND
        sc.exist('.//Identifier/ColumnReference[@Database=sql:variable("@dbname")][@Schema!="[sys]"]') = 1 AND
        sc.value('(@ScalarString)[1]' , 'varchar(128)') IS NOT NULL;
