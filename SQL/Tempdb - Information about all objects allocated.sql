...
Author: Eric Russell
Date: 22/09/2017
Description: Details about objects allocated in Tempdb
Original link: http://www.sqlservercentral.com/scripts/tempdb/151252/
Obs.: This must be run in context of TEMPDB database.
...
FROM    (
            ...
            JOIN sys.tables AS o ON o.object_id = ps.object_id
                                    AND o.is_ms_shipped = 0
            LEFT JOIN sys.indexes si ON si.object_id = o.object_id
            ...
            LEFT JOIN (
                      ...
                      ) trace ON trace.ObjectID = ps.object_id
                                 AND trace.DatabaseName = 'tempdb'
                                 AND MostRecentObjectReference = 1
            LEFT JOIN sys.dm_exec_requests AS er ON er.session_id = trace.SPID
