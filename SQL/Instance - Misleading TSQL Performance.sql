...
Date: 22/09/2017
Description: Misleading SQL Server Performance Counters
...
SELECT TOP 20
  ...
  , CASE
        ...
        ELSE CAST(qs.total_logical_reads / execution_count AS NUMERIC (30, 1))
    END AS [avg logical reads]
  , CAST(qs.total_worker_time / 1000. / 1000. AS NUMERIC (30, 1)) AS [cpu sec]
  , CASE
        WHEN execution_count = 0 THEN 0
        ELSE CAST(qs.total_worker_time / execution_count / 1000. / 1000. AS NUMERIC (30, 1))
    END AS [avg cpu sec]
  , CAST(qs.total_elapsed_time / 1000. / 1000. AS NUMERIC (30, 1)) AS [elapsed sec]
  , CASE
        WHEN execution_count = 0 THEN 0
        ELSE CAST(qs.total_elapsed_time / execution_count / 1000. / 1000. AS NUMERIC (30, 1))
