...
Author: Sean Smith
Date: 22/09/2017
Description: Breakdown Of All Indexes Contained Within A Database.
Original link: 
            ...
            SELECT  I.[object_id]
                  ...
                  , (
                        SELECT  N', ' + C.name + N' • '
                                ...
                                + ( CASE
                                        ...
                                        WHEN LOWER( TYPE_NAME( C.system_type_id )) NOT IN ( N'bigint', N'bit', N'date', N'datetime', N'datetime2'
                                                                                          , N'datetimeoffset', N'decimal', N'float', N'int', N'money', N'numeric'
                                                                                          , N'real', N'smalldatetime', N'smallint', N'smallmoney', N'time'
                                                                                          , N'tinyint'
                                                                                          ) THEN CONVERT( VARCHAR (6), C.max_length )
                                        ELSE
                  ...
                  , (
                        SELECT  N', ' + C.name + N' • ' + TYPE_NAME( C.user_type_id )
                                ...
                                + ( CASE
                                        WHEN TY.name NOT IN ( N'bigint', N'bit', N'date', N'datetime', N'datetime2', N'datetimeoffset', N'decimal', N'float'
                                                            , N'int', N'money', N'numeric', N'real', N'smalldatetime', N'smallint', N'smallmoney', N'time'
                                                            , N'tinyint'
                                        ...
                                        ELSE
                                            ...
                                            + ISNULL( CONVERT( NVARCHAR (30), COLUMNPROPERTY( C.[object_id], C.name, 'Scale' )), 0 ) + N')'
                                    END
                                  ) AS [text()]
                        FROM    sys.index_columns IC
                        INNER JOIN sys.columns C ON C.[object_id] = IC.[object_id]
...
SELECT  ( CASE
      ...
      , ( CASE
              WHEN sqBAQ.row_filter = 1 THEN
                  CONVERT( VARCHAR (6), CONVERT( DECIMAL (5, 2), ISNULL((( sqBAQ.index_pages + .0 ) / sqBAQ.used_pages ) * 100, 0 )))
              ELSE ''
          END
        ) AS pct_index
...
FROM    (
            ...
            INNER JOIN (
                           ...
                           FROM dbo.#temp_index_breakdown_size_info ttIBSI
...
LEFT JOIN (
              ...
              GROUP BY IC.[object_id]
                     , IC.index_id
          ) sqKL ON sqKL.[object_id] = sqBAQ.[object_id]
...
LEFT JOIN (
              ...
              FROM  (
                        ...
                        FROM    dbo.#temp_index_breakdown_keys_filters ttIBKF
