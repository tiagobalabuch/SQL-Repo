...
Date: 22/09/2017
Description: Finding tables with similar structures
    ...
    (   SELECT  c.TABLE_CATALOG
      ...
      */
        GROUP BY c.TABLE_CATALOG
               , c.TABLE_SCHEMA
               , c.TABLE_NAME )
SELECT  100 * COUNT( c2.COLUMN_NAME ) /*Matching_Column_Count*/ / MIN( ColCountsByTable.Column_Count ) /*Column_Count*/ AS Match_Percent
      , c.TABLE_CATALOG
      , c.TABLE_SCHEMA
      , c.TABLE_NAME
      , DENSE_RANK() OVER ( ORDER BY c.TABLE_CATALOG
                                   , c.TABLE_SCHEMA
                                   , c.TABLE_NAME
                          ) Table_Rank
...
INNER JOIN ColCountsByTable ON t.TABLE_CATALOG = ColCountsByTable.TABLE_CATALOG
                               AND  t.TABLE_SCHEMA = ColCountsByTable.TABLE_SCHEMA
                               AND  t.TABLE_NAME = ColCountsByTable.TABLE_NAME
LEFT OUTER JOIN INFORMATION_SCHEMA.COLUMNS c2 ON t.TABLE_NAME != c2.TABLE_NAME
