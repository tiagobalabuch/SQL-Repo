...
Date: 22/09/2017
Description: Get table hierarchy
...
SELECT  *
      , (
            SELECT TOP 1
                b.referencedcolumns
            FROM    @fkcolumns b
            WHERE   a.referencedtable = b.referencedtable
                    AND a.parenttable = b.parenttable
        )
      , STUFF((
...
WITH fks
   ( treelevel, treepath, tablename, referencedcolumns, parentcolumns )
AS
    (   SELECT  1
              ...
              , CAST('' AS VARCHAR (MAX))
        FROM    sys.tables a
        LEFT JOIN @fkrefs c ON a.name = c.parenttable
                               AND  c.referencedtable <> c.parenttable
