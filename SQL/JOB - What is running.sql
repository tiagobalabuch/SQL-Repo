...
Date: 22/09/2017
Description: What job is running 
...
INNER JOIN msdb.dbo.sysjobs j ON j.job_id = CONVERT(
                                                       UNIQUEIDENTIFIER
                                                     , CONVERT(
                                                                  BINARY (16)
                                                                , SUBSTRING(
                                                                               p.program_name
                                                                             , CHARINDEX( '(', p.program_name ) + 5
                                                                             , CHARINDEX( ':', p.program_name ) - CHARINDEX( '(', p.program_name ) - 6
                                                                           )
                                                                , 1
                                                              )
                                                   )
