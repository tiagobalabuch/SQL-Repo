...
Author: Tim Ford
Date: 22/09/2017
Description: Identify all logins with no associeted default database user.
...
IF EXISTS (
              ...
              WHERE name LIKE '#existing_db_users_table%'
          )
BEGIN
...
FETCH NEXT FROM cur_default_databases
INTO @database_name;

WHILE @@FETCH_STATUS = 0
BEGIN

    SELECT  @sql_text = 'INSERT INTO #existing_db_users_table(server_login_name, database_user_name, default_database_name)
...
FROM    sys.server_principals AS SP
LEFT JOIN #existing_db_users_table AS DDUT ON SP.name = DDUT.server_login_name
LEFT JOIN sys.databases AS D ON SP.default_database_name = D.name
...
IF EXISTS (
              SELECT    name
              FROM  tempdb.sys.tables
              WHERE name LIKE '#missing_existing_db_users_table%'
          )
BEGIN
