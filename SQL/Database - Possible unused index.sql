...
Author: Greg Larsen
Date: 22/09/2017
Description: Possible unused index
Original link: http://www.databasejournal.com/features/mssql/which-indexes-are-not-used.html
...
SELECT  o.name Object_Name
      ...
      , CASE
            WHEN (
                 ...
                 AND s.user_updates > 0 THEN 'USED AND UPDATED'
            WHEN (
                     s.user_seeks > 0
                     OR s.user_scans > 0
                     OR s.user_lookups > 0
                 )
                 AND s.user_updates = 0 THEN 'USED AND NOT UPDATED'
            WHEN s.user_seeks IS NULL
                 AND s.user_scans IS NULL
                 AND s.user_lookups IS NULL
                 AND s.user_updates IS NULL THEN 'NOT USED AND NOT UPDATED'
            WHEN (
                     s.user_seeks = 0
                     AND s.user_scans = 0
                     AND s.user_lookups = 0
                 )
                 AND s.user_updates > 0 THEN 'NOT USED AND UPDATED'
