...
SELECT  @diff AS HrsSinceFirstChange
      ...
      , tt.NTUserName
      , tt.ApplicationName
      , ( DENSE_RANK() OVER ( ORDER BY ObjectName
                                     , ObjectType
                            )
        ) % 2 AS l1
      , ( DENSE_RANK() OVER ( ORDER BY ObjectName
...
INNER JOIN sys.trace_subclass_values tsv ON tt.EventClass = tsv.trace_event_id
                                            AND tt.ObjectType = tsv.subclass_value
INNER JOIN master.dbo.spt_values sv ON tsv.subclass_value = sv.number
                                       AND  sv.type = 'EOD'
