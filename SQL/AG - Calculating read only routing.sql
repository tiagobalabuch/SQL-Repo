...
Date: 22/09/2017
Description:  it will tell you what the appropriate read_only_routing_url is for the instance.
Original link: https://blogs.msdn.microsoft.com/mattn/2012/04/25/calculating-read_only_routing_url-for-alwayson/
...
IF (
       @@microsoftversion / 0x01000000 < 11
       OR   @is_sql_azure > 0
   )
BEGIN
...
BEGIN -- if server supports read-only routing
    ...
    WHERE   type = 0 -- Type 0 = TSQL (to avoid mirroring endpoint) 
        ...
        ip_address NOT IN (
                              SELECT    ip_address
                              FROM  sys.availability_group_listener_ip_addresses agls
                          )
    GROUP BY port
